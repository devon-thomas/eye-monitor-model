...
# Apply an optional ideal uniform ADC quantizer to sampled values.
def apply_adc_quantization(vk, adc_cfg):
    if adc_cfg is None or not bool(adc_cfg.get("enabled", False)):
        return vk

    n_bits = int(adc_cfg.get("n_bits", 8))
    vmin = float(adc_cfg.get("vmin", -0.5))
    vmax = float(adc_cfg.get("vmax", 0.5))

    if n_bits <= 0:
        raise ValueError("adc.n_bits must be > 0")
    if vmax <= vmin:
        raise ValueError("adc.vmax must be greater than adc.vmin")

    levels = 2 ** n_bits
    step = (vmax - vmin) / float(levels)

    vkq = np.clip(vk, vmin, vmax - step)
    vkq = np.floor((vkq - vmin) / step) * step + vmin + 0.5 * step
    return vkq


# === SAMPLE_EYE ===
# Collect eye-monitor samples across commanded phases with optional jitter and noise.
def sample_eye(t, y, tbit, n_phases, n_ui_collect, sigma_j, sigma_v, skip_initial_ui, adc_cfg=None):
    ...
    for phi in phase_offsets:
        ui_idx_nom = np.arange(n_ui_collect) + k0
        tk_nom = ui_idx_nom * tbit + phi
        tk = tk_nom + np.random.randn(n_ui_collect) * sigma_j

        mask = (tk >= t[0]) & (tk <= t[-1])
        tk = tk[mask]
        ui_idx = ui_idx_nom[mask]
        ...
        vk = apply_adc_quantization(vk, adc_cfg)
        ...
        sample_ui_indices.append(ui_idx)
    ...
    return {
        ...
        "sample_ui_indices": sample_ui_indices,
    }
...
def _plot_actual_waveform_eye(t, y, tbit, phi_center, max_eye_segments, skip_initial_ui, n_ui_collect):
    t_ui = t / tbit
    dt_ui = t_ui[1] - t_ui[0]
    samples_per_ui = int(round(1.0 / dt_ui))
    seg_len = 2 * samples_per_ui

    start_ui = int(skip_initial_ui)
    n_segments_total = max(0, int(n_ui_collect))
    n_segments_plot = min(n_segments_total, max_eye_segments)

    waveform_label_added = False
    for k in range(n_segments_plot):
        ui_idx = start_ui + k
        i0 = ui_idx * samples_per_ui
        ...
        # Break the plotted line at modulo wrap boundaries so matplotlib
        # does not draw artificial straight segments across the eye.
        wrap_idx = np.where(np.abs(np.diff(x_seg)) > 1.0)[0]
...
def _plot_commanded_phase_sample_eye(results, phi_center):
    ...
    for phase_cmd, vals, ui_idx in zip(
        results["sample_phase_cmd_ui"],
        results["sample_values"],
        results["sample_ui_indices"]
    ):
        if len(vals) == 0:
            continue

        x1 = np.mod(phase_cmd[0] - phi_center + 1.0, 2.0)
        x2 = np.mod(x1 + 1.0, 2.0)

        even_mask = (ui_idx % 2) == 0
        odd_mask = ~even_mask

        if not sample_label_added:
            if np.any(even_mask):
                plt.scatter(
                    np.full(np.sum(even_mask), x1),
                    vals[even_mask],
                    s=4,
                    alpha=0.22,
...
# === _SUMMARY_TABLE_ROWS ===
# Build summary rows matching the terminal eye-metrics summary formatting.
def _summary_table_rows(metrics, tbit):
    return [
        ["Best phase (commanded)", "%.6f UI  (%.3f ps)" % (metrics["best_phase_ui"], metrics["best_phase_ui"] * tbit * 1e12)],
        ["Eye height @ best phase raw", "%.6f V" % metrics["eye_height_center_raw"]],
        ["Eye height @ best phase safe", "%.6f V" % metrics["eye_height_center_safe"]],
        ["Eye area raw", "%.6f V*UI" % metrics["eye_area_raw"]],
        ["Eye area safe", "%.6f V*UI" % metrics["eye_area_safe"]],
        ["Max eye height raw", "%.6f V" % metrics["eye_height_max_raw"]],
        ["Max eye height safe", "%.6f V" % metrics["eye_height_max_safe"]],
        ["Left eye edge", "%.6f UI  (%.3f ps)" % (metrics["left_edge_ui"], metrics["left_edge_ui"] * tbit * 1e12)],
        ["Right eye edge", "%.6f UI  (%.3f ps)" % (metrics["right_edge_ui"], metrics["right_edge_ui"] * tbit * 1e12)],
        ["Eye width", "%.6f UI  (%.3f ps)" % (metrics["eye_width_ui"], metrics["eye_width_ui"] * tbit * 1e12)],
    ]


# === _DRAW_FREQUENCY_RESPONSE ===
# Draw frequency response on the currently selected axes.
def _draw_frequency_response(cfg):
    tbit = float(cfg["data"]["tbit"])
    f_max = 1.0 / tbit
    n_points = int(cfg.get("debug", {}).get("frequency_plot_points", 1000))
    f_hz = np.linspace(0.0, f_max, n_points)
...
# === _DRAW_PULSE_RESPONSE_TIME ===
# Draw pulse response in time on the currently selected axes.
def _draw_pulse_response_time(sim):
    pulse_dense = sim["pulse_dense"]
    pulse_symbols = sim["pulse_symbols"]
    samples_per_bit = int(round(sim["tbit"] / sim["dt"]))

    if pulse_dense is not None:
        t_ui = np.arange(len(pulse_dense), dtype=float) / float(samples_per_bit)
        plt.plot(t_ui, pulse_dense)
        plt.xlabel("Time [UI]")
        plt.ylabel("Amplitude")
        plt.title("Pulse Response in Time")
        plt.grid(True)
        return

    if pulse_symbols is not None:
        t_ui = np.arange(len(pulse_symbols), dtype=float)
        plt.plot(t_ui, pulse_symbols, marker="o")
        plt.xlabel("Time [UI]")
        plt.ylabel("Amplitude")
        plt.title("Pulse Response in Time")
        plt.grid(True)
    ...
    plt.axis("off")
    plt.text(0.5, 0.5, "No pulse response available", ha="center", va="center", transform=plt.gca().transAxes)


# === _DRAW_WAVEFORM_SEGMENT ===
# Draw waveform segment on the currently selected axes.
def _draw_waveform_segment(sim, cfg):
    t = sim["t"]
    x_tx = sim["x_tx"]
    x_ch = sim["x_ch"]
    y = sim["y"]
...
# === _DRAW_EYE_DIAGRAM ===
# Draw reconstructed eye diagram on the currently selected axes.
def _draw_eye_diagram(sim, cfg):
    t = sim["t"]
    y = sim["y"]
    tbit = sim["tbit"]
    vcm = sim["vcm"]
    ...
    skip_initial_ui = int(cfg["sampling"]["skip_initial_ui"])
    n_ui_collect = int(cfg["sampling"]["n_ui_collect"])
    _plot_actual_waveform_eye(t, y, tbit, phi_center, max_eye_segments, skip_initial_ui, n_ui_collect)
...
# === _QFUNC ===
# Return the Gaussian tail probability Q(x) for scalar or array input.
def _qfunc(x):
    x = np.asarray(x, dtype=float)
    return 0.5 * np.vectorize(math.erfc)(x / np.sqrt(2.0))


# === _DRAW_RAIL_STATISTICS ===
# Draw centered rail statistics and eye opening on the currently selected axes.
def _draw_rail_statistics(metrics):
    ...
    std_upper_c = np.roll(metrics["std_upper"], shift)
    std_lower_c = np.roll(metrics["std_lower"], shift)
    opening_raw_c = np.roll(metrics["opening_raw"], shift)
    opening_safe_c = np.roll(metrics["opening_safe"], shift)

    ax = plt.gca()
    ax.plot(phase_centered, mu_upper_c, marker="o", label="Upper mean")
    ax.plot(phase_centered, mu_lower_c, marker="o", label="Lower mean")
    ax.plot(phase_centered, opening_raw_c, marker="o", label="Opening raw")
    ax.plot(phase_centered, opening_safe_c, marker="o", label="Opening safe")
    ax.fill_between(
        phase_centered,
        0.0,
    ...
    ax.axvline(0.5, linestyle="--", linewidth=1.0, alpha=0.8, label="Best phase")
    ax.axvline(metrics["left_edge_ui"], linestyle=":", linewidth=1.0, alpha=0.8, label="Left edge")
    ax.axvline(metrics["right_edge_ui"], linestyle=":", linewidth=1.0, alpha=0.8, label="Right edge")
    ax.set_xlim([0.0, 1.0])
    ax.set_xlabel("Commanded sample phase [UI] (centered)")
    ax.set_ylabel("Voltage [V]")
    ax.set_title("Rail Statistics, Eye Opening, and Bathtub Curve")
    ax.grid(True)

    # Approximate BER bathtub curve using midpoint threshold and Gaussian rail statistics.
    sigma_floor = 1e-15
    sigma_upper_eff = np.maximum(std_upper_c, sigma_floor)
    sigma_lower_eff = np.maximum(std_lower_c, sigma_floor)
    threshold_c = 0.5 * (mu_upper_c + mu_lower_c)

    q_arg_upper = (mu_upper_c - threshold_c) / sigma_upper_eff
    q_arg_lower = (threshold_c - mu_lower_c) / sigma_lower_eff
    ber_upper = _qfunc(q_arg_upper)
    ber_lower = _qfunc(q_arg_lower)
    bathtub_c = 0.5 * (ber_upper + ber_lower)
    bathtub_c = np.clip(bathtub_c, 1e-15, 1.0)

    ax2 = ax.twinx()
    ax2.semilogy(phase_centered, bathtub_c, marker="s", linestyle="-", label="Bathtub BER")
    ax2.set_ylabel("BER")
...
# === _DRAW_SUMMARY_TABLE ===
# Draw summary table matching terminal output on the currently selected axes.
def _draw_summary_table(metrics, tbit):
    ax = plt.gca()
    ax.axis("off")
    rows = _summary_table_rows(metrics, tbit)
    table = ax.table(
        cellText=rows,
    ...
    ax.set_title("Eye Metrics Summary")
...
def plot_results(sim, cfg):
    ...
    # -------------------------------------------------
    # Figure 1: frequency response, pulse response, waveform segment
    # -------------------------------------------------
    fig1, axes1 = plt.subplots(3, 1, figsize=(11, 13))

    plt.sca(axes1[0])
    if cfg.get("debug", {}).get("plot_frequency_response", False):
        _draw_frequency_response(cfg)
    else:
        axes1[0].axis("off")
        axes1[0].text(0.5, 0.5, "Frequency response plotting disabled", ha="center", va="center", transform=axes1[0].transAxes)

    plt.sca(axes1[1])
    _draw_pulse_response_time(sim)

    plt.sca(axes1[2])
    _draw_waveform_segment(sim, cfg)

    fig1.tight_layout()
    if save_plots:
        fig1.savefig(save_prefix + "_overview_signal.png", dpi=200)

    # -------------------------------------------------
    # Figure 2: eye diagram, rail statistics, summary table
    # -------------------------------------------------
    fig2, axes2 = plt.subplots(3, 1, figsize=(11, 14))

    plt.sca(axes2[0])
    _draw_eye_diagram(sim, cfg)

    plt.sca(axes2[1])
    _draw_rail_statistics(metrics)

    plt.sca(axes2[2])
    _draw_summary_table(metrics, tbit)

    fig2.tight_layout()
    if save_plots:
        fig2.savefig(save_prefix + "_overview_eye.png", dpi=200)

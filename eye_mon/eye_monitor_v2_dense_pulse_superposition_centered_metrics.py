...
def compute_eye_metrics(results, tbit, cfg_eye_metrics, vcm):
    ...
    # ---------------------------------------------------------
    # Recenter all phase-dependent metric arrays so best phase
    # appears at 0.5 UI. This makes eye-width search linear and
    # avoids wraparound issues near 0/1 UI.
    # ---------------------------------------------------------
    n = len(phase_cmd_ui)
    center_idx = n // 2
...
def plot_results(sim, cfg):
    ...
    plt.figure(figsize=(9, 5))
    phase_cmd_ui_centered = metrics["phase_cmd_ui_centered"]
    plt.plot(phase_cmd_ui_centered, metrics["mu_upper_centered"], marker="o", label="Upper mean")
    plt.plot(phase_cmd_ui_centered, metrics["mu_lower_centered"], marker="o", label="Lower mean")
    plt.plot(phase_cmd_ui_centered, metrics["opening_raw_centered"], marker="o", label="Opening raw")
    plt.plot(phase_cmd_ui_centered, metrics["opening_safe_centered"], marker="o", label="Opening safe")
    plt.fill_between(
        phase_cmd_ui_centered,
        0.0,

...
def compute_eye_metrics(results, tbit, cfg_eye_metrics, vcm):
    ...
    # ---------------------------------------------------------
    # Recenter all phase-dependent metric arrays so the best
    # phase appears at 0.5 UI. This avoids wrap-around issues
    # when the eye opening straddles 0/1 UI.
    # ---------------------------------------------------------
    n_phase = len(phase_cmd_ui)
    center_idx = n_phase // 2
    shift = center_idx - best_idx

    phase_cmd_ui_centered = (np.arange(n_phase) - center_idx) / float(n_phase) + 0.5
    mu_upper_centered = np.roll(mu_upper, shift)
    mu_lower_centered = np.roll(mu_lower, shift)
    std_upper_centered = np.roll(std_upper, shift)
    std_lower_centered = np.roll(std_lower, shift)
    n_upper_centered = np.roll(n_upper, shift)
    n_lower_centered = np.roll(n_lower, shift)
    opening_raw_centered = np.roll(opening_raw, shift)
    ...
    return {
        ...
        "phase_cmd_ui_centered": phase_cmd_ui_centered,
        "mu_upper": mu_upper,
        "mu_lower": mu_lower,
        "std_upper": std_upper,
        "std_lower": std_lower,
        "n_upper": n_upper,
        "n_lower": n_lower,
        "mu_upper_centered": mu_upper_centered,
        "mu_lower_centered": mu_lower_centered,
        "std_upper_centered": std_upper_centered,
        "std_lower_centered": std_lower_centered,
        "n_upper_centered": n_upper_centered,
        "n_lower_centered": n_lower_centered,
        "opening_raw": opening_raw,
        "opening_safe": opening_safe,
        "opening_raw_centered": opening_raw_centered,
        "opening_safe_centered": opening_safe_centered,
        "best_idx": best_idx,
...
def plot_results(sim, cfg):
    ...
    plt.figure(figsize=(9, 5))
    plt.plot(metrics["phase_cmd_ui_centered"], metrics["mu_upper_centered"], marker="o", label="Upper mean")
    plt.plot(metrics["phase_cmd_ui_centered"], metrics["mu_lower_centered"], marker="o", label="Lower mean")
    plt.plot(metrics["phase_cmd_ui_centered"], metrics["opening_raw_centered"], marker="o", label="Opening raw")
    plt.plot(metrics["phase_cmd_ui_centered"], metrics["opening_safe_centered"], marker="o", label="Opening safe")
    plt.fill_between(
        metrics["phase_cmd_ui_centered"],
        0.0,
        np.maximum(metrics["opening_safe_centered"], 0.0),
        alpha=0.2,
        label="Safe area"
    )
    ...
    plt.xlim([0.0, 1.0])
    plt.xlabel("Commanded sample phase [UI] (centered at best phase)")
    plt.ylabel("Voltage [V]")
...
def run_model(cfg):
    ...
    if input_waveform_model == "pulse_response_bits":
        ...
        pulse_dense = load_dense_pulse_response(input_waveform_cfg["file"])
        x_ch_full = apply_dense_pulse_superposition(eq_symbols, pulse_dense, samples_per_bit)
...
def main():
    config_path = "config_nominal.json"
    if len(sys.argv) > 1:
